...
public class ChooseAutoCommand implements Command {
    ...
    public View execute(IRequestWrapper wrapper) {
        ...
        final CarFilter carFilterLambda = carFilter;
        List<Car> cars = (List<Car>) daoManager.openExecuteAndClose(
                manager -> manager.getCarsDAO().filterAndGetCars(carFilterLambda));
        if (cars == null) {
            return View.INDEX;
        }
